...
Encode Encode::string(const std::string& str) {
    return Encode().appendValue(Decode::MT_string, str.size()).append(NW::data(str));
}
...
Encode Encode::closeIndefArray() {
    ...
    NW::append(_data, 0xFF);
    // close counter
    --openIndefCount;
    return *this;
}

Encode Encode::fromRaw(const NW::Data& rawData) {
...
Encode Encode::appendValue(byte majorType, uint64_t value) {
    ...
    // add bytes
    NW::append(_data, (byte)((majorType << 5) | (minorType & 0x1F)));
...
Decode::TypeDesc Decode::getTypeDesc() const {
    ...
    auto minorType = (NW::byte)((uint8_t)getByte(0) & 0x1F);
    if (minorType < 24) {
        // direct value
...
Data Decode::getBytes() const {
    ...
    return NW::data(data->origData.data() + (subStart + typeDesc.byteCount), len); 
}
...
vector<Decode> Decode::getCompoundElements(uint32_t countMultiplier, NW::byte expectedType) const {
    TypeDesc typeDesc = getTypeDesc();
...
{
    assert(subStart + subLen <= data->origData.size());
    return NW::data(data->origData.data() + subStart, subLen);
}
