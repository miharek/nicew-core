...
size_t paddingSize(size_t origSize, size_t blockSize, NWAESPaddingMode paddingMode) {
    if (origSize % blockSize == 0) {
        // even blocks
        if (paddingMode == NWAESPaddingModePKCS7) {
            return blockSize;
        }
        return 0;
    }
...
Data AESCBCEncrypt(const Data& key, const Data& data, Data& iv, NWAESPaddingMode paddingMode) {
    aes_encrypt_ctx ctx;
...
Data AESCBCDecrypt(const Data& key, const Data& data, Data& iv, NWAESPaddingMode paddingMode) {
    const size_t blockSize = AES_BLOCK_SIZE;
    ...
    if (paddingMode == NWAESPaddingModePKCS7 && result.size() > 0) {
        ...
        if (paddingSize <= result.size()) {
            ...
            Data resultUnpadded = NW::data(result.data(), unpaddedSize);
