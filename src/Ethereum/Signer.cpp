...
Signature Signer::signatureDataToStruct(const Data& signature, bool includeEip155, const uint256_t& chainID) noexcept {
    ...
    return signatureDataToStrucNWithEip155(chainID, signature);
}
...
Signature Signer::signatureDataToStrucNWithEip155(const uint256_t& chainID, const Data& signature) noexcept {
...
Signature Signer::sign(const PrivateKey& privateKey, const Data& hash, bool includeEip155, const uint256_t& chainID) noexcept {
    auto signature = privateKey.sign(hash, NWCurveSECP256k1);
...
Data addressStringToData(const std::string& asString) {
    ...
    if (asString.compare(0, 2, "0x") != 0) {
        return NW::addressToData(NWCoinTypeRonin, asString);
    }
    return NW::addressToData(NWCoinTypeEthereum, asString);
}
