...
public:
    ...
    static Encode fromRaw(const NW::Data& rawData);

private:
    Encode() {}
    Encode(const NW::Data& rawData) : _data(rawData) {}
    ...
    inline Encode append(const NW::Data& data) { NW::append(_data, data); return *this; }
...
private:
    ...
    /// Get the Nth byte
    inline NW::byte getByte(uint32_t idx) const {
    ...
    std::vector<Decode> getCompoundElements(uint32_t countMultiplier, NW::byte expectedType) const;
    bool isBreak() const;
