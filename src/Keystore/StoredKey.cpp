...
StoredKey StoredKey::createWithMnemonic(const std::string& name, const Data& password, const std::string& mnemonic, NWStoredKeyEncryptionLevel encryptionLevel) {
    if (!Mnemonic::isValid(mnemonic)) {
        throw std::invalid_argument("Invalid mnemonic");
    }

    Data mnemonicData = NW::Data(mnemonic.begin(), mnemonic.end());
    return StoredKey(StoredKeyType::mnemonicPhrase, name, password, mnemonicData, encryptionLevel);
}

StoredKey StoredKey::createWithMnemonicRandom(const std::string& name, const Data& password, NWStoredKeyEncryptionLevel encryptionLevel) {
    const auto wallet = NW::HDWallet(128, "");
...
StoredKey StoredKey::createWithMnemonicAddDefaultAddress(const std::string& name, const Data& password, const std::string& mnemonic, NWCoinType coin) {
    StoredKey key = createWithMnemonic(name, password, mnemonic, NWStoredKeyEncryptionLevelDefault);
...
StoredKey StoredKey::createWithPrivateKey(const std::string& name, const Data& password, const Data& privateKeyData) {
    return StoredKey(StoredKeyType::privateKey, name, password, privateKeyData, NWStoredKeyEncryptionLevelDefault);
}

StoredKey StoredKey::createWithPrivateKeyAddDefaultAddress(const std::string& name, const Data& password, NWCoinType coin, const Data& privateKeyData) {
    ...
    const auto derivationPath = NW::derivationPath(coin);
    const auto pubKeyType = NW::publicKeyType(coin);
    const auto pubKey = PrivateKey(privateKeyData).getPublicKey(pubKeyType);
    const auto address = NW::deriveAddress(coin, PrivateKey(privateKeyData));
    key.accounts.emplace_back(address, coin, NWDerivationDefault, derivationPath, hex(pubKey.bytes), "");
    return key;
}

StoredKey::StoredKey(StoredKeyType type, std::string name, const Data& password, const Data& data, NWStoredKeyEncryptionLevel encryptionLevel)
...
std::vector<Account> StoredKey::getAccounts(NWCoinType coin) const {
...
std::optional<Account> StoredKey::getDefaultAccount(NWCoinType coin, const HDWallet* wallet) const {
    ...
    for (auto& account : coinAccounts) {
        if (account.derivation == NWDerivationDefault) {
            return account;
        }
    }
    return std::nullopt;
...
std::optional<Account> StoredKey::getDefaultAccountOrAny(NWCoinType coin, const HDWallet* wallet) const {
...
std::optional<Account> StoredKey::getAccount(NWCoinType coin, const std::string& address) const {
    for (auto& account : accounts) {
...
std::optional<Account> StoredKey::getAccount(NWCoinType coin, NWDerivation derivation, const HDWallet& wallet) const {
    // obtain address
...
Account StoredKey::fillAddressIfMissing(Account& account, const HDWallet* wallet) const {
    ...
    if (account.publicKey.empty() && wallet != nullptr) {
        const auto pubKeyType = NW::publicKeyType(account.coin);
...
std::optional<const Account> StoredKey::account(NWCoinType coin, const HDWallet* wallet) {
    ...
    const auto derivationPath = NW::derivationPath(coin);
    const auto address = wallet->deriveAddress(coin);
    const auto version = NW::xpubVersion(coin);
    ...
    addAccount(address, coin, NWDerivationDefault, derivationPath, hex(pubKey.bytes), extendedPublicKey);
    return accounts.back();
}

Account StoredKey::account(NWCoinType coin, NWDerivation derivation, const HDWallet& wallet) {
    ...
    const auto address = wallet.deriveAddress(coin, derivation);
    const auto version = NW::xpubVersionDerivation(coin, derivation);
    const auto extendedPublicKey = wallet.getExtendedPublicKey(derivationPath.purpose(), coin, version);
    const auto pubKeyType = NW::publicKeyType(coin);
    const auto pubKey = wallet.getKey(coin, derivationPath).getPublicKey(pubKeyType);
...
std::optional<const Account> StoredKey::account(NWCoinType coin) const {
    return getDefaultAccountOrAny(coin, nullptr);
}

std::optional<const Account> StoredKey::account(NWCoinType coin, NWDerivation derivation, const HDWallet& wallet) const {
...
void StoredKey::removeAccount(NWCoinType coin) {
    accounts.erase(
        std::remove_if(accounts.begin(), accounts.end(), [coin](Account& account) -> bool { return account.coin == coin; }),
        accounts.end());
}

void StoredKey::removeAccount(NWCoinType coin, NWDerivation derivation) {
    accounts.erase(
...
void StoredKey::removeAccount(NWCoinType coin, DerivationPath derivationPath) {
    accounts.erase(
...
const PrivateKey StoredKey::privateKey(NWCoinType coin, const Data& password) {
    return privateKey(coin, NWDerivationDefault, password);
}

const PrivateKey StoredKey::privateKey(NWCoinType coin, [[maybe_unused]] NWDerivation derivation, const Data& password) {
    if (type == StoredKeyType::mnemonicPhrase) {
    ...
    case StoredKeyType::mnemonicPhrase: {
        ...
        for (auto& account : accounts) {
            if (!account.address.empty() && !account.publicKey.empty() &&
                NW::validateAddress(account.coin, account.address)) {
                continue;
            }
            const auto& derivationPath = account.derivationPath;
            const auto key = wallet.getKey(account.coin, derivationPath);
            const auto pubKey = key.getPublicKey(NW::publicKeyType(account.coin));
            account.address = NW::deriveAddress(account.coin, pubKey, account.derivation);
            account.publicKey = hex(pubKey.bytes);
        }
    } break;
...
void StoredKey::loadJson(const nlohmann::json& json) {
    ...
    if (accounts.empty() && json.count(CodingKeys::address) != 0 &&
        ...
        NWCoinType coin = NWCoinTypeEthereum;
        if (json.count(CodingKeys::coin) != 0) {
            coin = json[CodingKeys::coin].get<NWCoinType>();
        }
        auto address = json[CodingKeys::address].get<std::string>();
        accounts.emplace_back(address, coin, NWDerivationDefault, DerivationPath(NWPurposeBIP44, NWCoinTypeSlip44Id(coin), 0, 0, 0), "", "");
    }
}
