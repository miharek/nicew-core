...
public:
    ...
    static StoredKey createWithMnemonic(const std::string& name, const Data& password, const std::string& mnemonic, NWStoredKeyEncryptionLevel encryptionLevel);

    /// Create a new StoredKey, with the given name, mnemonic and password.
    /// @throws std::invalid_argument if mnemonic is invalid
    static StoredKey createWithMnemonicRandom(const std::string& name, const Data& password, NWStoredKeyEncryptionLevel encryptionLevel);

    /// Create a new StoredKey, with the given name, mnemonic and password, and also add the default address for the given coin..
    /// @throws std::invalid_argument if mnemonic is invalid
    static StoredKey createWithMnemonicAddDefaultAddress(const std::string& name, const Data& password, const std::string& mnemonic, NWCoinType coin);
    ...
    /// @throws std::invalid_argument if privateKeyData is not a valid private key
    static StoredKey createWithPrivateKeyAddDefaultAddress(const std::string& name, const Data& password, NWCoinType coin, const Data& privateKeyData);
    ...
    std::vector<Account> getAccounts(NWCoinType coin) const;
    ...
    std::optional<const Account> account(NWCoinType coin, const HDWallet* wallet);
    ...
    Account account(NWCoinType coin, NWDerivation derivation, const HDWallet& wallet);
    ...
    std::optional<const Account> account(NWCoinType coin) const;
    
    /// Returns the account for a specific coin and derivation, if it exists.
    std::optional<const Account> account(NWCoinType coin, NWDerivation derivation, const HDWallet& wallet) const;
    ...
    void removeAccount(NWCoinType coin);

    /// Remove the account for a specific coin with the given derivation.
    void removeAccount(NWCoinType coin, NWDerivation derivation);

    /// Remove the account for a specific coin with the given derivation path.
    void removeAccount(NWCoinType coin, DerivationPath derivationPath);
    ...
    const PrivateKey privateKey(NWCoinType coin, const Data& password);

    /// Returns the private key for a specific coin, creating an account if necessary.
    ///
    /// @throws std::invalid_argument if this key is of a type other than
    /// `mnemonicPhrase` and a coin other than the default is requested.
    const PrivateKey privateKey(NWCoinType coin, NWDerivation derivation, const Data& password);
...
private:
    ...
    /// parameters.
    StoredKey(StoredKeyType type, std::string name, const Data& password, const Data& data, NWStoredKeyEncryptionLevel encryptionLevel);
    ...
    std::optional<Account> getDefaultAccount(NWCoinType coin, const HDWallet* wallet) const;

    /// Find account for coin, if exists.  If multiple exist, default is returned, or any.
    /// Optional wallet is needed to derive default address
    std::optional<Account> getDefaultAccountOrAny(NWCoinType coin, const HDWallet* wallet) const;

    /// Find account by coin+address (should be one, if multiple, first is returned)
    std::optional<Account> getAccount(NWCoinType coin, const std::string& address) const;

    /// Find account by coin+derivation (should be one, if multiple, first is returned)
    std::optional<Account> getAccount(NWCoinType coin, NWDerivation derivation, const HDWallet& wallet) const;
