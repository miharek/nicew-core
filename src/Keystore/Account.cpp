...
Account::Account(const nlohmann::json& json) {
    if (json.find(CodingKeys::derivation) != json.end()) {
        derivation = NWDerivation(json[CodingKeys::derivation].get<uint32_t>());
    }
    ...
    if (json.find(CodingKeys::coin) == json.end()) {
        ...
        coin = NWCoinType(uint32_t(derivationPath.indices[1].value));
    } else {
        coin = NWCoinType(json[CodingKeys::coin].get<uint32_t>());
    }
...
nlohmann::json Account::json() const {
    ...
    if (derivation != NWDerivationDefault) {
