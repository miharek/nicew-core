...
public:
    ...
    NW::Hash::Hasher hasher = NW::Hash::HasherSha256d;
...
public:
    ...
    Transaction(int32_t version, uint32_t lockTime = 0, NW::Hash::Hasher hasher = NW::Hash::HasherSha256d)
    ...
    Data getPreImage(const Script& scriptCode, size_t index, enum NWBitcoinSigHashType hashType, uint64_t amount) const;
    ...
    Data getSignatureHash(const Script& scriptCode, size_t index, enum NWBitcoinSigHashType hashType,
                          uint64_t amount, enum SignatureVersion version) const;

    void serializeInput(size_t subindex, const Script&, size_t index, enum NWBitcoinSigHashType hashType, Data& data) const;
...
private:
    ...
    Data getSignatureHashWitnessV0(const Script& scriptCode, size_t index,
                                   enum NWBitcoinSigHashType hashType, uint64_t amount) const;

    /// Generates the signature hash for for scripts other than witness scripts.
    Data getSignatureHashBase(const Script& scriptCode, size_t index,
                              enum NWBitcoinSigHashType hashType) const;
};
