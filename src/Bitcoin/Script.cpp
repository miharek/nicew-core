...
bool Script::matchMultisig(std::vector<Data>& keys, int& required) const {
    ...
    if (!op || !NWOpCodeIsSmallInteger(opcode)) {
        return false;
    }
    ...
    while (true) {
        ...
        if (!PublicKey::isValid(operand, NWPublicKeyTypeSECP256k1)) {
            break;
        }
        keys.push_back(operand);
    }

    if (!NWOpCodeIsSmallInteger(opcode)) {
        return false;
    }
...
Script Script::lockScriptForAddress(const std::string& string, enum NWCoinType coin) {
