...
public:
    ...
    static Transaction build(const TransactionPlan& plan, const std::string& toAddress,
                             const std::string& changeAddress, enum NWCoinType coin, uint32_t lockTime) {
        Transaction tx;
        tx.lockTime = lockTime;

        auto outputTo = prepareOutpuNWithScript(toAddress, plan.amount, coin);
        if (!outputTo.has_value()) { return {}; }
        tx.outputs.push_back(outputTo.value());

        if (plan.change > 0) {
            auto outputChange = prepareOutpuNWithScript(changeAddress, plan.change, coin);
    ...
    static std::optional<TransactionOutput> prepareOutpuNWithScript(std::string address, Amount amount, enum NWCoinType coin);
