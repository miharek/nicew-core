...
std::optional<TransactionOutput> TransactionBuilder::prepareOutpuNWithScript(std::string address, Amount amount, enum NWCoinType coin) {
...
int64_t estimateSegwitFee(const FeeCalculator& feeCalculator, const TransactionPlan& plan, int outputSize, const SigningInput& input) {
    NWPurpose coinPurpose = NW::purpose(static_cast<NWCoinType>(input.coinType));
    if (coinPurpose != NWPurposeBIP84) {
    ...
    auto inpuNWithPlan = std::move(input);
    inpuNWithPlan.plan = plan;

    auto result = TransactionSigner<Transaction, TransactionBuilder>::sign(inpuNWithPlan, SigningMode_SizeEstimationOnly);
    if (!result) {
...
TransactionPlan TransactionBuilder::plan(const SigningInput& input) {
    ...
    if (input.amount == 0 && !maxAmount) {
        ...
        const auto& feeCalculator = getFeeCalculator(static_cast<NWCoinType>(input.coinType));
