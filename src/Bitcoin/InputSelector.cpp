...
InputSelector<TypeWithAmount>::select(uint64_t targetValue, uint64_t byteFee, uint64_t numOutputs) {
    ...
    for (size_t numInputs = 1; numInputs <= n; ++numInputs) {
        ...
        const auto targeNWithFeeAndDust = targetValue + fee + dustThreshold;
        if (maxWithXInputs[numInputs] < targeNWithFeeAndDust) {
        ...
        slices.erase(
            std::remove_if(slices.begin(), slices.end(),
                           [targeNWithFeeAndDust](const std::vector<TypeWithAmount>& slice) {
                               return sum(slice) < targeNWithFeeAndDust;
                           }),
            slices.end());
        if (!slices.empty()) {
    ...
    for (size_t numInputs = 1; numInputs <= n; ++numInputs) {
        const auto fee = feeCalculator.calculate(numInputs, numOutputs, byteFee);
        const auto targeNWithFee = targetValue + fee;
        if (maxWithXInputs[numInputs] < targeNWithFee) {
            // no way to satisfy with only numInputs inputs, skip
            continue;
        }
        auto slices = slice(sorted, static_cast<size_t>(numInputs));
        slices.erase(std::remove_if(slices.begin(), slices.end(),
                                    [targeNWithFee](const std::vector<TypeWithAmount>& slice) {
                                        return sum(slice) < targeNWithFee;
                                    }),
                     slices.end());
        if (!slices.empty()) {
            return filterOutDust(slices.front(), byteFee);
