...
class HDWallet {
  ...
  public:
    ...
    const NW::Data& getEntropy() const { return entropy; }
  ...
  public:
    ...
    /// Returns master key.
    PrivateKey getMasterKey(NWCurve curve) const;

    /// Returns the master private key extension (32 byte).
    PrivateKey getMasterKeyExtension(NWCurve curve) const;

    /// Returns the private key with the given derivation.
    PrivateKey getKey(const NWCoinType coin, NWDerivation derivation) const;

    /// Returns the private key at the given derivation path.
    PrivateKey getKey(const NWCoinType coin, const DerivationPath& derivationPath) const;

    /// Derives the address for a coin (default derivation).
    std::string deriveAddress(NWCoinType coin) const;

    /// Derives the address for a coin with given derivation.
    std::string deriveAddress(NWCoinType coin, NWDerivation derivation) const;

    /// Returns the extended private key for default 0 account with the given derivation.
    std::string getExtendedPrivateKeyDerivation(NWPurpose purpose, NWCoinType coin, NWDerivation derivation, NWHDVersion version) const {
    ...
    std::string getExtendedPublicKeyDerivation(NWPurpose purpose, NWCoinType coin, NWDerivation derivation, NWHDVersion version) const {
    ...
    std::string getExtendedPrivateKey(NWPurpose purpose, NWCoinType coin, NWHDVersion version) const {
        return getExtendedPrivateKeyAccount(purpose, coin, NWDerivationDefault, version, 0);
    }

    /// Returns the extended public key for default 0 account; derivation path used is "m/purpose'/coin'/0'".
    std::string getExtendedPublicKey(NWPurpose purpose, NWCoinType coin, NWHDVersion version) const {
        return getExtendedPublicKeyAccount(purpose, coin, NWDerivationDefault, version, 0);
    }

    /// Returns the extended private key for a custom account; derivation path used is "m/purpose'/coin'/account'".
    std::string getExtendedPrivateKeyAccount(NWPurpose purpose, NWCoinType coin, NWDerivation derivation, NWHDVersion version, uint32_t account) const;

    /// Returns the extended public key for a custom account; derivation path used is "m/purpose'/coin'/account'".
    std::string getExtendedPublicKeyAccount(NWPurpose purpose, NWCoinType coin, NWDerivation derivation, NWHDVersion version, uint32_t account) const;

    /// Returns the BIP32 Root Key (private)
    std::string getRootKey(NWCoinType coin, NWHDVersion version) const;

    /// Computes the public key from an extended public key representation.
    static std::optional<PublicKey> getPublicKeyFromExtended(const std::string& extended, NWCoinType coin, const DerivationPath& path);

    /// Computes the private key from an extended private key representation.
    static std::optional<PrivateKey> getPrivateKeyFromExtended(const std::string& extended, NWCoinType coin, const DerivationPath& path);
