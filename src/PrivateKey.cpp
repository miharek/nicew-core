...
bool PrivateKey::isValid(const Data& data, NWCurve curve) {
    // check size
...
NWPrivateKeyType PrivateKey::getType(NWCurve curve) noexcept {
    switch (curve) {
    case NWCurve::NWCurveED25519ExtendedCardano:
        return NWPrivateKeyTypeCardano;
    default:
        return NWPrivateKeyTypeDefault;
    }
}
...
PublicKey PrivateKey::getPublicKey(NWPublicKeyType type) const {
    Data result;
    switch (type) {
    ...
    case NWPublicKeyTypeCURVE25519:
        result.resize(PublicKey::ed25519Size);
        PublicKey ed25519PublicKey = getPublicKey(NWPublicKeyTypeED25519);
...
Data PrivateKey::getSharedKey(const PublicKey& pubKey, NWCurve curve) const {
    ...
    if (success) {
        PublicKey sharedKey(result, NWPublicKeyTypeSECP256k1Extended);
...
Data PrivateKey::sign(const Data& digest, NWCurve curve) const {
    Data result;
    bool success = false;
    switch (curve) {
    case NWCurveSECP256k1: {
    ...
    case NWCurveCurve25519: {
        result.resize(64);
        const auto publicKey = getPublicKey(NWPublicKeyTypeED25519);
        ed25519_sign(digest.data(), digest.size(), key().data(), result.data());
...
Data PrivateKey::sign(const Data& digest, NWCurve curve, int (*canonicalChecker)(uint8_t by, uint8_t sig[64])) const {
    Data result;
    bool success = false;
    switch (curve) {
    case NWCurveSECP256k1: {
    ...
    case NWCurveED25519:                // not supported
    case NWCurveED25519Blake2bNano:     // not supported
    case NWCurveED25519ExtendedCardano: // not supported
    case NWCurveCurve25519:             // not supported
        break;
    case NWCurveNIST256p1: {
        result.resize(65);
