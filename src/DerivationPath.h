...
struct DerivationPath {
    ...
    NWPurpose purpose() const {
        if (indices.size() == 0) { return NWPurposeBIP44; }
        return static_cast<NWPurpose>(indices[0].value);
    }

    void setPurpose(NWPurpose v) {
        if (indices.size() == 0) { return; }
        indices[0] = DerivationPathIndex(v, /* hardened: */ true);
    }

    uint32_t coin() const {
        if (indices.size() <= 1) { return NWCoinTypeBitcoin; }
    ...
    DerivationPath(NWPurpose purpose, uint32_t coin, uint32_t account, uint32_t change,
                   uint32_t address) 
