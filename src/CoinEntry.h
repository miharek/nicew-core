...
public:
    ...
    virtual bool validateAddress(NWCoinType coin, const std::string& address, NW::byte p2pkh, NW::byte p2sh, const char* hrp) const = 0;
    // normalizeAddress is optional, it may leave this default, no-change implementation
    virtual std::string normalizeAddress([[maybe_unused]] NWCoinType coin, const std::string& address) const { return address; }
    // Address derivation, default derivation
    virtual std::string deriveAddress(NWCoinType coin, const PublicKey& publicKey, NW::byte p2pkh, const char* hrp) const = 0;
    // Address derivation, by default invoking default
    virtual std::string deriveAddress(NWCoinType coin, [[maybe_unused]] NWDerivation derivation, const PublicKey& publicKey, NW::byte p2pkh, const char* hrp) const {
    ...
    virtual Data addressToData([[maybe_unused]] NWCoinType coin, [[maybe_unused]] const std::string& address) const { return {}; }
    // Signing
    virtual void sign(NWCoinType coin, const Data& dataIn, Data& dataOut) const = 0;
    virtual bool supportsJSONSigning() const { return false; }
    // It is optional, Signing JSON input with private key
    virtual std::string signJSON([[maybe_unused]] NWCoinType coin, [[maybe_unused]] const std::string& json, [[maybe_unused]] const Data& key) const { return ""; }
    ...
    virtual void plan([[maybe_unused]] NWCoinType coin, [[maybe_unused]] const Data& dataIn, [[maybe_unused]] Data& dataOut) const { }
    ...
    virtual Data preImageHashes([[maybe_unused]] NWCoinType coin, [[maybe_unused]] const Data& txInputData) const { return {}; }
    // Optional method for compiling a transaction with externally-supplied signatures & pubkeys.
    virtual void compile([[maybe_unused]] NWCoinType coin, [[maybe_unused]] const Data& txInputData, [[maybe_unused]] const std::vector<Data>& signatures, [[maybe_unused]] const std::vector<PublicKey>& publicKeys, [[maybe_unused]] Data& dataOut) const {}
    ...
    virtual Data buildTransactionInput([[maybe_unused]] NWCoinType coinType, [[maybe_unused]] const std::string& from, [[maybe_unused]] const std::string& to, [[maybe_unused]] const uint256_t& amount, [[maybe_unused]] const std::string& asset, [[maybe_unused]] const std::string& memo, [[maybe_unused]] const std::string& chainId) const { return Data(); }
};
...
Data txCompilerTemplate(const Data& dataIn, Func&& fnHandler) {
    ...
    if (!input.ParseFromArray(dataIn.data(), (int)dataIn.size())) {
        ...
        return NW::data(output.SerializeAsString());;
    }
