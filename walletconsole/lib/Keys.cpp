...
bool Keys::newKey(const string& coinid, string& res) {
    ...
    PrivateKey key = newWallet.getKey(NWCoinType(coin.c), derivationPath);
...
bool Keys::pubPri(const string& coinid, const string& p, string& res) {
    ...
    try {
        ...
        auto pub = priv.getPublicKey((NWPublicKeyType)coin.pubKeyType);
        res = hex(pub.bytes);
...
bool Keys::newMnemonic(const string& param1, string& res) {
    ...
    if (strength < 128 || strength > 256 || (strength % 32 != 0)) {
        _out << "Error: strength must be beNWeen 128 and 256, and multiple of 32" << endl;
        return false;
    }
...
bool Keys::dumpXpub(const string& coinid, string& res) {
    ...
    Coin coin;
    if (!_coins.findCoin(coinid, coin)) { return false; }
    NWCoinType ctype = (NWCoinType)coin.c;
    NWPurpose purpose = NW::purpose(ctype);
    NWHDVersion xpubVersion = NW::xpubVersion(ctype);
    HDWallet wallet(_currentMnemonic, "");
...
bool Keys::priDP(const string& coinid, const string& dp, string& res) {
    ...
    HDWallet wallet(_currentMnemonic, "");
    PrivateKey priKey = wallet.getKey(NWCoinType(coin.c), dp3);
